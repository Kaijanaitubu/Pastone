tyrano.plugin.kag = {
    ...
    cache_scenario: {},
    event_listener_map: {},
    array_white_attr: ["id", "src", "name", "data-event-tag", "data-event-pm", "data-event-target", "data-event-storage", "tabindex", "l_visible", "data-parent-layer", "data-video-name", "data-video-pm", "data-effect", "data", "type"],
    config: {
    ...
    removeSaveData: function () {
      const project_id = this.kag.config.projectID,
        ...
        suffixes = ["_sf", "_tyrano_data", "_tyrano_quick_save", "_tyrano_auto_save"];
    ...
    init_game: function () {
      ...
      $("body").on("fullscreenchange", (e => {
        document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement || document.fullScreenElement || !1 ? this.kag.trigger("fullscreen-start", e) : this.kag.trigger("fullscreen-stop", e)
      }));
      ...
      if (this.config.keyFocusOutlineAnim && "none" !== this.config.keyFocusOutlineAnim) {
        switch (this.config.keyFocusOutlineAnim) {
          default:
          case "flash":
            $.insertRuleToTyranoCSS(":focus.focus { animation: focus 1000ms infinite alternate linear; }");$.insertRuleToTyranoCSS(`\n                    @keyframes focus {\n                        0%   { outline-color: ${focus_outline_color}; }\n                        3%   { outline-color: ${focus_outline_color}; }\n                        97%  { outline-color: transparent; }\n                        100% { outline-color: transparent; }\n                    }`);
          break;
        case "flash_momentary":
            $.insertRuleToTyranoCSS(":focus.focus { animation: focus 1000ms infinite steps(1, end); }");$.insertRuleToTyranoCSS(`\n                    @keyframes focus {\n                        0% { outline-color: ${focus_outline_color}; }\n                        50%, 100% { outline-color: transparent; }\n                    }`)
        }
        this.config.keyFocusOutlineAnimDuration && $.insertRuleToTyranoCSS(`:focus.hover { animation-duration: ${this.config.keyFocusOutlineAnimDuration}ms; }`)
      }
      "true" === this.config.keyFocusWithHoverStyle && $.copyHoverCSSToFocusCSS('link[href*="tyrano/tyrano.css"]');
    ...
    setNewParagraph: function (j_inner) {
      "true" == this.stat.vertical ? j_inner.append("<p class='vertical_text'></p>") : j_inner.append("<p class=''></p>")
    },
    ...
    preload: function (src, callbk, options = {}) {
      ...
      if ("wav" == ext || "mp3" == ext || "ogg" == ext || "m4a" == ext) {
        ...
        if (preloaded_audio) switch (preloaded_audio.state()) {
        case "unload":
        ...
        audio_obj.once("loaderror", (() => {
          audio_obj.unload();
          this.kag.error("preload_failure_sound", {
            src: src
          });
          onend(audio_obj);
          delete this.kag.tmp.preload_audio_map[src]
        ...
        audio_obj.__single_use = void 0 === options.single_use || options.single_use;
        const names = (options.name || "").split(",").map((item => item.trim()));
        audio_obj.__names = names;
        audio_obj.load()
      } else "mp4" == ext || "ogv" == ext || "webm" == ext ? $("<video />").on("loadeddata", (function (e) {
        onend(this)
      })).on("error", (function (e) {
        that.kag.error("preload_failure_video", {
          src: src
        });
        onend()
      })).attr("src", src) : $("<img />").on("load", (function (e) {
        onend(this)
      })).on("error", (function (e) {
        that.kag.error("preload_failure_image", {
          src: src
        });
        onend()
      })).attr("src", src)
    ...
    preloadAll: function (storage, callbk) {
      ...
      if (Array.isArray(storage)) {
        ...
        let sum = 0;
        for (let i = 0; i < storage.length; i++) that.kag.preload(storage[i], (() => {
          sum++;
    ...
    preloadNextVoice: function () {
      const array_tag = this.kag.ftag.array_tag,
        end_index = Math.min(array_tag.length, this.kag.ftag.current_order_index + 20);
    ...
    setStyles: function (j_obj, array_style) {
      for (let key in array_style) void 0 !== array_style[key] && ("" === array_style[key] || j_obj.css(key, array_style[key]));
      return j_obj
    },
    ...
    logTrigger: function (event_name, event_obj) {
      ...
      console.log("%c" + event_name, `padding: 4px 6px; border-radius: 4px; background: #000; color: ${color};`);
    ...
    trigger: function (event_name, event_obj = {}) {
      ...
      for (const listener of map[event_name]) {
        ...
        "function" == typeof listener.callback && (callback_return_value = listener.callback.call(this, event_obj));
        if (listener.once) {
          exists_once = !0;
      ...
      if (exists_once) {
        const new_listeners = [];
        for (const listener of map[event_name]) unbind_target_ids.includes(listener.id) || new_listeners.push(listener);
    ...
    sortEventLisneners: function (evnet_name) {
      ...
      Array.isArray(listeners) && listeners.sort(((a, b) => a.priority > b.priority ? -1 : a.priority < b.priority ? 1 : a.id < b.id ? -1 : 1))
    },
    ...
    offTempListeners: function () {
      const map = this.event_listener_map;
      for (const event in map) map[event] && (map[event] = map[event].filter((listener => !listener.temp)));
      this.is_event_logging_enabled && this.logEventLisnenerCount()
    },
    ...
    parseKeyframesForWebAnimationAPI: function (name) {
      if (!this.stat.map_keyframe[name] || !this.stat.map_keyframe[name].frames) return null;
      ...
      for (const percentage_str in frames) {
        ...
        transform_strs.length > 0 && (this_keyframe.transform = transform_strs.join(" "));
        for (const _key in frame.styles) {
          if ("_tag" === _key) continue;
          this_keyframe[$.parseCamelCaseCSS(_key)] = $.convertColor(frame.styles[_key])
        }
    ...
    changeHowlVolume: function (audio_obj, options = {}) {
      let tag_volume = void 0 !== audio_obj.__tag_volume ? audio_obj.__tag_volume : 1,
        config_volume = void 0 !== audio_obj.__config_volume ? audio_obj.__config_volume : 1;
    ...
    makeFocusable: function (j_elm, tabindex = 0) {
      if ("false" !== this.config.useKeyFocus) {
        ...
        j_elm.on("focusin", (() => {
          "true" === this.config.keyFocusWithHoverStyle && j_elm.trigger("mouseenter");
          j_elm.addClass("focus")
        }));
        j_elm.on("focusout", (() => {
          "true" === this.config.keyFocusWithHoverStyle && j_elm.trigger("mouseleave");
    ...
    makeUnfocusableAll: function (j_elm) {
      "false" !== this.config.useKeyFocus && this.makeUnfocusable(j_elm.find("[tabindex]"))
    },
    unfocus: function () {
      $(":focus").blur().removeClass("hover")
    },
    restoreFocusable: function () {
      "false" !== this.config.useKeyFocus && $(".tyrano-focusable").each(((i, elm) => {
        const j_elm = $(elm),
    ...
    chara: {
      ...
      getCharaName() {
        ...
        if ("" != this.kag.stat.chara_ptext) {
          ...
          chara_name = j_chara_name.hasClass("multiple-text") ? j_chara_name.find(".fill").text() : $.isNull(j_chara_name.html())
        }
        return chara_name
      },
      ...
      setPartContainer(j_chara) {
        this.isPlusLighterEnabled() && j_chara.children("img").each(((i, img) => {
    ...
    showModeEffect(_type, options = {}) {
      ...
      this.tmp.screen_effect_timer_id = setTimeout((() => {
        ...
        if ("default" === storage) {
          ...
          def.width && "auto" !== def.width && j_effect.css("font-size", `${(def.width/15).toFixed(0)}px`);
          ...
          if (def.color)
            ...
            } else j_effect.children().css("border-left-color", $.convertColor(def.color))
        } else {
          const src = $.parseStorage(storage, "image");
          j_effect = $(`<img id="mode_effect" src="${src}" class="mode_effect ${type}" />`);
          def.width && "auto" !== def.width && j_effect.css("width", $.convertLength(def.width));
          def.height && "auto" !== def.height && j_effect.css("height", $.convertLength(def.height))
        }
    ...
    showLoadingLog(type = "preload") {
      ...
      if (text && "none" !== text) {
        "default" === text && (text = this.kag.getTag("loading_log").default_message_map[type]);
        "notext" === text && (text = "");
        tmp.j_loading_log_message.text(text);
        text && tmp.j_loading_log_message.setStyle("animation-duration", `${this.kag.stat.loading_log.dot_time}ms`);
        this.kag.stat.loading_log.use_icon ? tmp.j_loading_log_icon.show() : tmp.j_loading_log_icon.hide();
