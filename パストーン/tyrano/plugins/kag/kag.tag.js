tyrano.plugin.kag.ftag = {
    ...
    restoreNextImg: function () {
      ...
      if (0 !== $("." + (is_fixed ? "glyph_image" : "img_next")).length && this.kag.stat.glyph_pm)
        if (is_fixed) {
          const pm = $.extend({}, this.kag.stat.glyph_pm, {
            next: "false"
          });
    ...
    createNextImg: function (mode = "") {
      ...
      const class_names = [];
      let j_glyph, img_src, id = "";
      mode ? id = "mode_glyph_" + mode : "true" !== pm.fix ? class_names.push("img_next") : class_names.push("glyph_image");
      switch (pm.type) {
        default:
        case "image":
          img_src = $.parseStorage(pm.line || "nextpage.gif", pm.folder);j_glyph = $(`<img src="${img_src}">`);pm.width && j_glyph.setStyle("width", pm.width + "px");pm.height && j_glyph.setStyle("height", pm.height + "px");
        break;
      case "html":
          j_glyph = $(`<div>${pm.html}</div>`);pm.width && j_glyph.setStyle("width", pm.width + "px");pm.height ? j_glyph.setStyle("height", pm.height + "px") : pm.width && j_glyph.setStyle("height", pm.width + "px");
        break;
      case "figure":
          pm.figure && class_names.push("img_next_" + pm.figure);j_glyph = $("<div></div>");pm.color && j_glyph.setStyle("background-color", $.convertColor(pm.color));pm.width && j_glyph.setStyle("width", pm.width + "px");pm.height ? j_glyph.setStyle("height", pm.height + "px") : pm.width && j_glyph.setStyle("height", pm.width + "px");
        break;
      case "koma_anim":
        {
          img_src = $.parseStorage(pm.koma_anim, pm.folder);j_glyph = $("<div></div>");
          const j_koma_anim = $("<div></div>");j_koma_anim.setStyleMap({
      ...
      if (pm.keyframe) j_glyph.animateWithTyranoKeyframes(pm);
        ...
        pm.time && j_glyph.setStyle("animation-duration", $.convertDuration(pm.time));
        pm.delay && j_glyph.setStyle("animation-delay", $.convertDuration(pm.delay));
    ...
    nextOrder: function () {
      ...
      if (1 == this.checkCond(tag)) {
        ...
        if (this.master_tag[tag.name]) {
          this.kag.stat.is_script || !0 === tag.is_entity_disabled || (tag.pm = this.convertEntity(tag.pm));
    ...
    checkVital: function (tag) {
      ...
      for (var array_vital = master_tag.vital, i = 0; i < array_vital.length; i++) tag.pm[array_vital[i]] ? "" == tag.pm[array_vital[i]] && (err_str = $.lang("missing_parameter", {
        tag: tag.name,
        param: array_vital[i]
    ...
    nextOrderWithLabel: function (label_name, scenario_file) {
      ...
      if ("*savesnap" != label_name) {
        ...
        if (scenario_file != this.kag.stat.current_scenario && null != original_scenario) {
          ...
          this.kag.error("undefined_label", {
            name: label_name
          });
          this.nextOrder()
        }
    ...
    nextOrderWithIndex: function (index, scenario_file, flag, insert, auto_next) {
      ...
      if ((scenario_file = scenario_file || this.kag.stat.current_scenario) != this.kag.stat.current_scenario || 1 == flag) {
        ...
        nextorder_called || this.kag.stat.is_strong_stop || this.kag.layer.showEventLayer()
      }
    }
  };
  tyrano.plugin.kag.tag.text = {
    ...
    getMessageConfig: function (key) {
      return (this.kag.stat.message_config || {})[key] || this.default_message_config[key]
    },
    start: function (pm) {
      ...
      this.kag.trigger("tag-text-message", {
        target: pm
      });
      ...
      "false" != this.kag.config.defaultAutoReturn && this.autoInsertPageBreak(j_inner_message, j_outer_message, is_vertical);
    ...
    setMessageInnerStyle: function (j_inner_message) {
      const font_feature_settings = "true" === this.getMessageConfig("kerning") ? '"palt"' : "initial";
    ...
    autoInsertPageBreak: function (j_inner_message, j_outer_message, is_vertical) {
      ...
      var limit_width = .8 * parseInt(j_outer_message.css(target_property));
      parseInt(j_inner_message.find("p").css(target_property)) > limit_width && (this.kag.stat.vchat.is_active ? this.kag.ftag.startTag("vchat_in", {}) : this.kag.getMessageInnerLayer().html(""))
    },
    showMessage: function (message_str, is_vertical) {
      ...
      const j_span = this.kag.getMessageCurrentSpan(),
        tmp = this.kag.tmp;
      tmp.should_set_reverse_indent = chara_name && !j_span.text() && "false" !== this.getMessageConfig("speech_bracket_float");
      const font = this.kag.stat.font;
      void 0 !== font.effect && "none" !== font.effect || (font.effect = "");
      void 0 !== font.gradient && "none" !== font.gradient || (font.gradient = "");
      tmp.is_text_stroke = font.edge && "stroke" === font.edge_method;
      tmp.is_edge_overlap = "true" === this.getMessageConfig("edge_overlap_text");
      tmp.is_individual_decoration = tmp.is_text_stroke || font.edge && "shadow" === font.edge_method && (!tmp.is_edge_overlap || font.gradient);
      ...
      "" != font.effect && "fadeIn" != font.effect || (should_use_inline_block = !1);
    ...
    speechMessage: function (message_str) {
      ...
      this.kag.tmp.speak_on_volume && (utterance.volume = this.kag.tmp.speak_on_volume);
      this.kag.tmp.speak_on_pitch && (utterance.pitch = this.kag.tmp.speak_on_pitch);
      this.kag.tmp.speak_on_rate && (utterance.rate = this.kag.tmp.speak_on_rate);
      this.kag.tmp.speak_on_utterance && this.kag.tmp.speak_on_cancel && speechSynthesis.cancel(this.kag.tmp.speak_on_utterance);
    ...
    setCurrentSpanStyle: function (j_span, chara_name) {
      if (this.kag.stat.vchat.is_active)
        ...
        const letter_spacing = this.getMessageConfig("letter_spacing") || this.kag.config.defaultPitch,
          line_spacing = this.getMessageConfig("line_spacing") || this.kag.config.defaultLineSpacing,
          ...
          switch (font.edge_method) {
            ...
            case "shadow":
              ...
              for (let i = edges.length - 1; i >= 0; i--) {
                const edge = edges[i],
                  text_shadow_value = $.generateTextShadowStrokeCSSOne(edge.color, edge.total_width);
                this.kag.tmp.text_shadow_values.push(text_shadow_value)
              }
              this.kag.tmp.inside_stroke_color = edges[0].color
            } else j_span.setStyle("text-shadow", $.generateTextShadowStrokeCSS(edge_str));
            break;
          case "filter":
        ...
        } else "" != font.shadow && j_span.setStyle("text-shadow", "2px 2px 2px " + font.shadow)
      }
    },
    manageAlreadyRead: function (j_span) {
      1 == this.kag.stat.already_read ? "default" != this.kag.config.alreadyReadTextColor && j_span.setStyle("color", $.convertColor(this.kag.config.alreadyReadTextColor)) : "false" == this.kag.config.unReadTextSkip && this.kag.setSkip(!1)
    },
    ...
    buildTextShadowChar: function (c, edge_str, is_visible = !1) {
      ...
      char_html += `<span class="char text-shadow ${is_visible?"visible":""}" style="${is_edge_overlap?"z-index: 10; opacity: 0; ":""}">`;
      const opacity_style = is_edge_overlap ? "opacity: 1; " : "";
      this.kag.tmp.text_shadow_values.forEach(((text_shadow_value, i, arr) => {
        const z_index = 11 + i,
          color_style = i + 1 < arr.length ? "" : `color: ${this.kag.tmp.inside_stroke_color}; `;
    ...
    buildTextStrokeChar: function (c, edge_str, is_visible = !1) {
      ...
      char_html += is_edge_overlap ? `<span class="char text-stroke ${visible_class}" style="z-index:10;opacity:0;">` : `<span class="char text-stroke ${visible_class}">`;
      char_html += `<span class="dummy" style="transform: scale(2);">${c}</span>`;
      for (let i = edges.length - 1; i >= 0; i--) {
        const edge = edges[i],
          width = 2 * edge.total_width;
        let style = `-webkit-text-stroke: ${width}px ${edge.color}; z-index: ${100-i}; padding: ${width}px; margin: -${width}px 0 0 -${width}px;`;
        is_edge_overlap && (style += "opacity:1;");
        char_html += `<span class="stroke entity" style="${style}">${c}</span>`
      }
      char_html += `<span class="fill entity" style="${is_edge_overlap?"opacity:1;":""}">${c}</span>`;
      char_html += `<span class="dummy" style="position:relative;display:inline;">${c}</span>`;
      return char_html + "</span>"
    },
    adjustFukiSize: function (j_msg_inner, chara_jname) {
      ...
      let chara_obj, chara_name = chara_jname;
      this.kag.stat.jcharas[chara_jname] && (chara_name = this.kag.stat.jcharas[chara_jname]);
      try {
      ...
      void 0 !== chara_obj && chara_obj.get(0) && "true" === this.kag.stat.charas[chara_name].fuki.enable ? this.adjustCharaFukiSize(j_msg_inner, chara_name, chara_obj) : this.adjustOthersFukiSize(j_msg_inner)
    },
    ...
    adjustOthersFukiSize: function (j_msg_inner) {
      ...
      this.kag.updateFuki("others", {
        sippo: "none"
      })
    },
    makeOneCharVisible: function (j_char_span) {
      this.kag.tmp.is_individual_decoration && !this.kag.tmp.is_edge_overlap && (j_char_span = j_char_span.find(".entity"));
      ...
      } else j_char_span.setStyleMap({
        visibility: "visible",
        opacity: "1"
      })
    },
    makeAllCharsVisible: function (j_char_span_children) {
      this.kag.tmp.is_individual_decoration && (j_char_span_children = j_char_span_children.find(".entity"));
    ...
    addOneChar: function (char_index, j_char_span_children, j_message_span, j_msg_inner) {
      ...
      next_char_index < j_char_span_children.length ? $.setTimeout((() => {
        this.addOneChar(next_char_index, j_char_span_children, j_message_span, j_msg_inner)
      }), this.kag.tmp.ch_speed) : $.setTimeout((() => {
    ...
    checkClickInterrupt: function (j_msg_inner) {
      ...
      if ((this.kag.stat.is_click_text || is_skip) && !this.kag.tmp.processed_click_interrupt) {
        ...
        "default" !== ch_speed_in_click && (this.kag.tmp.ch_speed = parseInt(ch_speed_in_click));
    ...
    finishAddingChars: function () {
      ...
      this.kag.stat.is_hide_message ? this.kag.once("messagewindow-show", (() => {
        this.kag.ftag.nextOrder()
      }), {
        temp: !0,
        system: !0
    ...
    addChars: function (j_message_span, j_msg_inner, is_vertical) {
      let ch_speed = 30;
      "" !== this.kag.stat.ch_speed ? ch_speed = parseInt(this.kag.stat.ch_speed) : this.kag.config.chSpeed && (ch_speed = parseInt(this.kag.config.chSpeed));
      const j_char_span_children = j_message_span.find(".char"),
        font = this.kag.stat.font;
      if (font.gradient && "none" !== font.gradient) {
        (this.kag.tmp.is_individual_decoration ? j_char_span_children.find(".fill") : j_char_span_children).setGradientText(font.gradient)
      }
      this.kag.tmp.should_set_reverse_indent && this.setReverseIndent(j_msg_inner, j_char_span_children);
      "true" === this.getMessageConfig("control_line_break") && this.controlLineBreak(j_char_span_children, is_vertical);
      if (!0 === this.kag.stat.is_skip || this.kag.stat.is_nowait || ch_speed <= 3) {
    ...
    setReverseIndent: function (j_msg_inner, j_children) {
      ...
      0 !== margin && j_msg_inner.find("p").setStyleMap({
  ...
  tyrano.plugin.kag.tag.position = {
    ...
    start: function (pm) {
      ...
      "" !== pm.visible && ("true" === pm.visible ? this.kag.layer.showLayer(j_message_layer) : this.kag.layer.hideLayer(j_message_layer));
      ...
      "" !== pm.radius && (new_style_outer["border-radius"] = parseInt(pm.radius) + "px");
      if ("" !== pm.border_size) {
        new_style_outer["border-width"] = parseInt(pm.border_size) + "px";
        j_message_outer.css("border-style", "solid")
      }
      "" !== pm.border_color && (new_style_outer["border-color"] = $.convertColor(pm.border_color));
      "" !== pm.opacity && (new_style_outer.opacity = $.convertOpacity(pm.opacity));
      if ("" !== pm.color) {
      ...
      this.kag.stat.fuki.def_style = $.extend(!0, this.kag.stat.fuki.def_style, new_style_outer);
      const j_filter = j_message_layer.find(".message_filter");
      j_filter.length > 0 && ["left", "top", "width", "height", "border-radius", "border-style", "border-width"].forEach((key => {
      ...
      if ("" !== pm.margin) {
        ...
        switch (hash.length) {
          ...
          break;
        case 2:
            pm.margint = pm.marginb = hash[0];pm.marginl = pm.marginr = hash[1];
          break;
        case 3:
            pm.margint = hash[0];pm.marginl = pm.marginr = hash[1];pm.marginb = hash[2];
          break;
        case 4:
            pm.margint = hash[0];pm.marginr = hash[1];pm.marginb = hash[2];pm.marginl = hash[3]
        }
      }
      "" !== pm.marginl && (new_style_inner["padding-left"] = parseInt(pm.marginl) + "px");
      "" !== pm.margint && (new_style_inner["padding-top"] = parseInt(pm.margint) + "px");
      if ("" !== pm.marginr) {
  ...
  tyrano.plugin.kag.tag.fuki_chara = {
    ...
    start: function (pm) {
      ...
      if ("others" == pm.name) this.kag.stat.fuki.others_style = $.extend(this.kag.stat.fuki.others_style, pm);
      else {
  ...
  tyrano.plugin.kag.tag.ptext = {
    ...
    start: function (pm) {
      ...
      "" == pm.color ? pm.color = $.convertColor(font.color) : pm.color = $.convertColor(pm.color);
      ...
      let is_individual_decoration = is_edge_enabled && "stroke" === pm.edge_method;
      is_edge_enabled && "shadow" === pm.edge_method && pm.gradient && (is_individual_decoration = !0);
      if (is_edge_enabled) switch (pm.edge_method) {
      case "shadow":
        is_individual_decoration || tobj.css("text-shadow", $.generateTextShadowStrokeCSS(pm.edge));
        break;
      case "filter":
        tobj.setFilterCSS($.generateDropShadowStrokeCSS(pm.edge))
      } else "" != pm.shadow && tobj.css("text-shadow", "2px 2px 2px " + $.convertColor(pm.shadow));
      ...
      pm.gradient && !is_individual_decoration && tobj.setGradientText(pm.gradient);
      const target_layer = this.kag.layer.getLayer(pm.layer, pm.page);
      if ("" != pm.time) {
        tobj.css("opacity", 0);
        target_layer.append(tobj);
        tobj.stop(!0, !0).animate({
          opacity: 1
        }, parseInt(pm.time), (function () {
          that.kag.ftag.nextOrder()
        }))
    ...
    setEvent: function (j_target, pm) {
      ...
      j_target.get(0).updateText = str => {
        ...
        const inner_html = Array.prototype.reduce.call(str, ((total_html, this_char) => is_shadow ? total_html + that.kag.getTag("text").buildTextShadowChar(this_char, pm.edge, !0) : total_html + that.kag.getTag("text").buildTextStrokeChar(this_char, pm.edge, !0)), "");
  ...
  tyrano.plugin.kag.tag.mtext = {
    ...
    start: function (pm) {
      ...
      "fix" == pm.layer && tobj.addClass("fixlayer");
      target_layer.append(tobj);
      for (let key in pm) "true" == pm[key] ? pm[key] = !0 : "false" == pm[key] && (pm[key] = !1);
      tobj.textillate({
        loop: pm.fadeout,
    ...
    setEvent: function (j_span, pm) {
      ...
      j_span.on("click", (e => {
        ...
        if (!clicked || !once) {
          ...
          "true" == that.kag.stat.skip_link ? e.stopPropagation() : that.kag.setSkip(!1);
          return !1
        }
      }));
      that.kag.setElmCursor(j_span, "pointer")
  ...
  tyrano.plugin.kag.tag.s = {
    start: function () {
      ...
      this.kag.weaklyStop();
      this.kag.stat.glink_config && "true" === this.kag.stat.glink_config.auto_place && this.showGLinks()
    },
    showGLinks: function () {
      ...
      "default" !== glink_config.margin_y && j_glink_collection.setStyleMap({
        "margin-top": `${glink_config.margin_y}px`,
        "margin-bottom": `${glink_config.margin_y}px`
      });
      "default" !== glink_config.margin_x && j_glink_collection.setStyleMap({
        "margin-left": `${glink_config.margin_x}px`,
        "margin-right": `${glink_config.margin_x}px`
      });
      "default" !== glink_config.padding_y && j_glink_collection.setStyleMap({
        "padding-top": `${glink_config.padding_y}px`,
        "padding-bottom": `${glink_config.padding_y}px`
      });
      "default" !== glink_config.padding_x && j_glink_collection.setStyleMap({
      ...
      let area_nums;
      if ("auto" === glink_config.place_area) $.extend(wrapper_style, this.calcFlexPosition(glink_config));
      else if ("cover" === glink_config.place_area) $.extend(wrapper_style, {
        left: "0",
        top: "0",
        width: "100%",
        height: "100%"
        ...
        area_nums = glink_config.place_area.split(",").map((item => $.trim(item)));
      ...
      j_glink_collection.each(((i, elm) => {
        const j_elm = $(elm),
          ...
          need_animate = void 0 !== _pm.show_time && parseInt(_pm.show_time) >= 10 && ("none" !== _pm.show_keyframe || "none" !== _pm.show_effect);
      ...
      j_glink_collection.each(((i, elm) => {
        ...
        $.setTimeout((() => {
          j_elm.setStyle("visibility", "visible");
          if (_pm.show_keyframe && "none" !== _pm.show_keyframe) j_elm.animateWithTyranoKeyframes({
            ...
            onend: anim => {
              anim.cancel();
              showed_counter += 1;
              showed_counter === animation_target_count && j_wrapper.setStyleMap({
                "pointer-events": "auto"
              })
            }
          });
            ...
            _pm.show_time && j_elm.setStyle("animation-duration", $.convertDuration(glink_config.show_time));
            _pm.show_easing && j_elm.setStyle("animation-timing-function", glink_config.show_easing);
    ...
    calcFlexPosition: function (glink_config) {
      ...
      if ("none" === j_message_layer.css("display")) return {
        left: "0",
        top: "0",
        width: "100%",
        height: "100%"
  ...
  tyrano.plugin.kag.tag.quake2 = {
    ...
    start: function (pm) {
      ...
      if (this.kag.stat.is_skip && "true" === pm.skippable) return this.kag.ftag.nextOrder();
      ...
      const loop = () => {
        if (current_frame < end_frame) {
          if (current_frame % ignore_rate == 0) {
            ...
            let v = 0,
              h = 0;
            vmax > 0 && (v = sign * $.easing.easeOutQuad(null, current_frame, vmax, -vmax, end_frame));
            hmax > 0 && (h = sign * $.easing.easeOutQuad(null, current_frame, hmax, -hmax, end_frame));
  ...
  tyrano.plugin.kag.tag.vibrate = {
    ...
    start: function (pm) {
      ...
      time = pm.time.includes(",") ? pm.time.split(",").map((item => parseInt(item))) : duration;
      if (pm.count) {
        let new_time = [];
        if ("number" == typeof time) {
          ...
          for (let i = 0; i < count; i++) new_time = new_time.concat(time.concat())
        }
        time = new_time
      }
      try {
        this.kag.key_mouse.gamepad.last_used_next_gamepad_index > -1 ? this.kag.key_mouse.gamepad.vibrate({
          duration: time,
          power: power
  ...
  tyrano.plugin.kag.tag.vibrate_stop = {
    start: function (pm) {
      this.kag.key_mouse.gamepad.vibrate({
        duration: 0,
        power: 0
      });
      navigator.vibrate(0);
  ...
  tyrano.plugin.kag.tag.message_config = {
    ...
    start: function (pm) {
      ...
      for (const key in default_message_config) key in pm && (message_config[key] = pm[key]);
      this.kag.stat.word_nobreak_list || (this.kag.stat.word_nobreak_list = []);
      const list = this.kag.stat.word_nobreak_list;
      $.isNonEmptyStr(pm.add_word_nobreak) && pm.add_word_nobreak.split(",").forEach((word => {
  ...
  tyrano.plugin.kag.tag.button = {
    ...
    setEvent: function (j_button, pm) {
      ...
      pm.autoimg && this.kag.stat.is_auto && j_button.attr("src", $.parseStorage(pm.autoimg, pm.folder));
      pm.skipimg && this.kag.stat.is_skip && j_button.attr("src", $.parseStorage(pm.skipimg, pm.folder));
      ...
      const is_fix_button = "true" === pm.fix,
        ...
        is_save_button = "save" == pm.role || "menu" == pm.role || "quicksave" == pm.role || "sleepgame" == pm.role,
      ...
      j_button.hover((() => {
        ...
        j_button.hasClass("src-change-disabled") || pm.enterimg && j_button.attr("src", $.parseStorage(pm.enterimg, pm.folder));
        pm.enterse && this.kag.playSound(pm.enterse)
      }), (() => {
        if (!is_fix_button && !this.kag.stat.is_strong_stop) return !1;
        if (!is_fix_button && button_clicked) return !1;
        j_button.hasClass("src-change-disabled") || pm.enterimg && j_button.attr("src", $.parseStorage(pm.graphic, pm.folder));
      ...
      j_button.on("mousedown touchstart", (() => {
        ...
        j_button.hasClass("src-change-disabled") || pm.activeimg && j_button.attr("src", $.parseStorage(pm.activeimg, pm.folder));
        return !1
      }));
      j_button.click((e => {
        that.kag.tmp.ready_audio || that.kag.readyAudio();
        ...
        if (!that.kag.stat.is_strong_stop && "none" === that.kag.layer.layer_event.css("display")) return !1;
        if ("true" === pm.savesnap && that.kag.stat.is_stop) return !1;
        if (is_save_button && (that.kag.stat.is_adding_text || that.kag.stat.is_wait)) return !1;
        if ("sleepgame" === pm.role && null !== that.kag.tmp.sleep_game) return !1;
        ...
        "" != pm.clickimg ? j_button.attr("src", $.parseStorage(pm.clickimg, pm.folder)) : "" != pm.activeimg && j_button.attr("src", $.parseStorage(pm.graphic, pm.folder));
        pm.clickse && this.kag.playSound(pm.clickse);
        pm.exp && this.kag.embScript(pm.exp, preexp);
        "true" === pm.savesnap && that.kag.menu.snapSave(that.kag.stat.current_save_str);
        if (is_jump_button) {
        ...
        if (is_role_button) {
          ...
          that.kag.setSkip(!1);
          "auto" !== pm.role && that.kag.ftag.startTag("autostop", {
            next: "false"
          });
          switch (pm.role) {
          case "save":
          ...
          case "skip":
            if (that.kag.stat.is_skip) that.kag.setSkip(!1);
              ...
              that.kag.layer.layer_event.isDisplayed() && that.kag.layer.layer_event.click();
              that.kag.setSkip(!0)
            }
            break;
          case "backlog":
  ...
  tyrano.plugin.kag.tag.glink_config = {
    ...
    getConfig: function (name) {
      this.kag.stat.glink_config || (this.kag.stat.glink_config = $.extend({}, this.default_glink_config));
    ...
    start: function (pm) {
      ...
      for (const key in pm) "_tag" !== key && pm[key] && (this.kag.stat.glink_config[key] = pm[key]);
      this.kag.ftag.nextOrder()
    }
  };
  tyrano.plugin.kag.tag.glink = {
    ...
    start: function (pm) {
      ...
      pm.edge ? j_button.css("text-shadow", $.generateTextShadowStrokeCSS(pm.edge)) : pm.shadow && j_button.css("text-shadow", "2px 2px 2px " + $.convertColor(pm.shadow));
      if ("" != pm.graphic) {
      ...
      this.setEvent(j_button, pm);
      if ("true" === glink_config.auto_place_force || "true" === glink_config.auto_place && "auto" === pm.x && !pm.y) {
        ...
        if (void 0 !== show_options.time && parseInt(show_options.time) >= 10 && ("none" !== show_options.keyframe || "none" !== show_options.effect)) {
          show_options.callback = () => {
            j_button.setStyleMap({
              "pointer-events": "auto"
            })
          };
    ...
    setEvent: function (j_button, pm) {
      ...
      j_button.hover((() => {
        if (!this.kag.stat.is_strong_stop) return !1;
        if (button_clicked) return !1;
        pm.enterimg && j_button.css("background-image", "url(./data/image/" + pm.enterimg + ")");
        pm.enterse && this.kag.playSound(pm.enterse)
      }), (() => {
        if (!this.kag.stat.is_strong_stop) return !1;
        if (button_clicked) return !1;
        pm.enterimg && j_button.css("background-image", "url(./data/image/" + pm.graphic + ")");
        pm.leavese && this.kag.playSound(pm.leavese)
      }));
      ...
      j_button.click((e => {
        ...
        pm.clickimg && j_button.css("background-image", "url(./data/image/" + pm.clickimg + ")");
        this.kag.trigger("click-tag-glink", e);
        pm.clickse && this.kag.playSound(pm.clickse);
        pm.exp && this.kag.embScript(pm.exp, preexp);
        const next = () => {
          use_cm && this.kag.ftag.startTag("cm", {
            next: "false"
          });
          if ("true" === pm.hold) {
          ...
          "true" === this.kag.stat.skip_link ? e.stopPropagation() : this.kag.setSkip(!1)
        };
        let animation_target_count = 0;
        ...
        j_collection.each(((i, elm) => {
          ...
          const need_animate = void 0 !== hide_options.time && parseInt(hide_options.time) >= 10 && ("none" !== hide_options.keyframe || "none" !== hide_options.effect);
          need_animate && (animation_target_count += 1);
          elm.__hide_options = hide_options;
          elm.__need_animate = need_animate
        }));
        const should_keep_skip = this.kag.stat.is_skip && "true" === this.kag.stat.skip_link;
    ...
    startAnim: function (j_collection, options, do_hide) {
      j_collection.setStyleMap({
        "pointer-events": "none"
      });
      options.keyframe && "none" !== options.keyframe ? j_collection.each(((i, elm) => {
        ...
        const j_elm = $(elm);
        j_elm.setStyle("animation-fill-mode", "forwards");
        options.time && j_elm.setStyle("animation-duration", $.convertDuration(options.time));
        options.delay && j_elm.setStyle("animation-delay", $.convertDuration(options.delay));
        options.easing && j_elm.setStyle("animation-timing-function", options.easing);
        j_elm.on("animationend", (e => {
          if (j_elm.get(0) === e.target) {
            j_elm.off("animationend");
  ...
  tyrano.plugin.kag.tag.clickable = {
    ...
    setEvent: function (j_button, pm) {
      const that = this;
      let button_clicked = !1;
      pm.mouseopacity && j_button.hover((() => {
  ...
  tyrano.plugin.kag.tag.glyph = {
    ...
    start: function (pm) {
      ...
      let j_fix_glyph, is_fix_glyph_displayed = !1;
      switch (pm.target) {
        default:
        case "":
      ...
      pm.figure ? pm.type = "figure" : pm.html ? pm.type = "html" : pm.koma_anim ? pm.type = "koma_anim" : pm.type = "image";
      pm.keyframe && (pm.anim = "");
      ...
      pm.koma_anim ? this.kag.preload($.parseStorage(pm.koma_anim, pm.folder), (img => {
        if (img) {
  ...
  tyrano.plugin.kag.tag.trans = {
    ...
    start: function (pm) {
      ...
      for (let key in map_layer_fore) 1 != pm.children && key !== pm.layer || function () {
        var _key = key,
